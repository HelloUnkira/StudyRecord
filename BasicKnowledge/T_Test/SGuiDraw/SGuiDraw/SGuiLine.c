...
static uint8_t SGuiDrawSimpleLine(SGuiLine line)
{
    ...
    if (line.x_start == line.x_end) {
        ...
        for (i = start; i <= end; i++)
            SGUIDRAWDOT(line.instance, line.instance.color, x, i);
        return true;
    }
    /* 确认是水平线 */
    if (line.y_start == line.y_end) {
        ...
        for (i = start; i <= end; i++)
            SGUIDRAWDOT(line.instance, line.instance.color, i, y);
        return true;
    }
    /* 确认是对角线 */
    if (SGUIDIS(line.x_start, line.x_end) ==
        ...
        if (line.x_start < line.x_end && line.y_start < line.y_end) {
            for (i = 0; i <= step; i++)
                SGUIDRAWDOT(line.instance, line.instance.color, x++, y++);
        }
        if (line.x_start > line.x_end && line.y_start > line.y_end) {
            for (i = 0; i <= step; i++)
                SGUIDRAWDOT(line.instance, line.instance.color, x--, y--);
...
{
    /* 绘制:垂直线,水平线,对角线 */
    if (SGuiDrawSimpleLine(line) == true)
        return;

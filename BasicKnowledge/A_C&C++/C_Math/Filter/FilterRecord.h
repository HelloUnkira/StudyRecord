 ...
 *优点:对(温度,液位)等变化缓慢的被测参数有良好的滤波效果
 *     能有效克服因偶然因素引起的波动干扰
 *缺点:对(流量,速度)等快速变化的被测参数不宜
 */
/* --3-- */
/*算术平均滤波法
 *连续取N个采样值进行算术平均运算：
 ...
 *如果采样值==当前有效值,则计数器清零
 *如果采样值<>当前有效值,则计数器+1
 *如果计数器溢出一个指定值,则将本次值替换当前有效值,并清计数器

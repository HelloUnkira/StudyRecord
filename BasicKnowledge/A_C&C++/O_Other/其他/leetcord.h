...
void search_target_sum(int* data, int left, int right, int target)
{
	int n = abs(right - left), i = 0;
	struct hash_node** help= \
		(struct hash_node**)malloc(sizeof(struct hash_node*) * n);
	bool* used_data= (bool*)malloc(sizeof(bool) * n);	
	for (i = 0; i < n; i++) 
		help[i] = NULL;
	//将target - data[x] 散列到散列表之中即可
	//散列函数选abs(target - data[x]) % n ，其中将x保留下来即可
	for (i = left; i <= right; i++) {
		...
		//插入到散列索引对应的链表中
		//将
		if (help[abs(target - data[i]) % n] == NULL) {
			help[abs(target - data[i]) % n] = buf;			
		} else {
			struct hash_node* temp = help[abs(target - data[i]) % n];
			help[abs(target - data[i]) % n] = buf;	
			buf->next = temp;
		}		
	}
	//第二次遍历，对每个数，散列查找是否遇到所需要的值即可
	for (i = left; i <= right; i++) {
		if (help[abs(data[i]) % n] == NULL)
			continue;
		else {
			struct hash_node* temp = help[abs(data[i]) % n];
			while (temp != NULL) {
				if ((temp->index != i) && \
					((data[i] + data[temp->index]) == target) && \
					used_data[temp->index - left] == 0) {
						printf("%d - %d \n",i , temp->index);					
						used_data[i - left] = 1;
				}					
				temp = temp->next;
	...
	for (i = 0; i < n; i++) {
		if (help[i] != NULL) {		
			struct hash_node* temp = help[i];
			struct hash_node* temp1 = NULL;
			while (temp != NULL) {				
				temp1 = temp->next;
				free(temp);
				temp = temp1;
...
//求字符串中最长不重复子串
void max_length_patition_str(char * str)
{
	int max = 0;
	char* max_index = str;
	int current_max = 0;
	...
	while (*window != '\0') {	
		current_max++;
		if (used_char[*window - 'a'] == 0) {
			used_char[*window - 'a'] = 1;						
			if (current_max > max) {
				max_index = current_index;
				max = current_max;							
			}
		} else {			
			while (*current_index != *window) {
				used_char[*current_index - 'a'] = 0;
				current_index++;	
				current_max--;
			}
			current_index++;
...
//求俩个正序数组的中位数，时间复杂度为O(log(m+n))
//归并只能保证其时间复杂度在O(m+n)中
//一种特殊的求第k个最小数
float patition_search_median_number_from_two_arrays(\
	int* list1, int* list2, int left1, int right1, int left2, int right2, int k)
{
	...
	if (left1 > right1) 
		...
		if (left1 + k / 2 > right1)
			return patition_search_median_number_from_two_arrays(\
				list1, list2, left1, right1, left2 + k / 2, right2, k - k / 2);
		else if (left2 + k / 2 > right2)
			return patition_search_median_number_from_two_arrays(\
				list1, list2, left1 + k / 2, right1, left2, right2, k - k / 2);
		else if (list1[left1 + k / 2] >= list2[left2 + k / 2])
			return patition_search_median_number_from_two_arrays(\
				list1, list2, left1, right1, left2 + k / 2, right2, k - k / 2);	
		else	if (list2[left2 + k / 2] > list1[left1 + k / 2])
			return patition_search_median_number_from_two_arrays(\
				list1, list2, left1 + k / 2, right1, left2, right2, k - k / 2);		
	}
}

float search_median_number_from_two_arrays(\
	int* list1, int* list2, int left1, int right1, int left2, int right2)
{
	int k = (right1 - left1 + 1 + right2 - left2 + 1) / 2;
	return patition_search_median_number_from_two_arrays(\
	list1, list2, left1, right1, left2, right2, k);	
}
...
void search_max_length_palindrome_str1(char* str)
{
	int count = 0, i = 0, j = 0;
	char* my_str = str;
	while (*my_str++ != '\0')
	...
	my_str = str;
	//申请空间
	char* help =(char*)malloc(sizeof(char) * (count + 1));
	int** dp = (int**)malloc(sizeof(int *) * (count + 1));
	for (i = 0; i < count + 1; i++)
		dp[i] = (int*)malloc(sizeof(int) * (count + 1));
	//初始化
	for (i = 0; i < count + 1; i++) {
		for (j = 0; j < count + 1; j++)
			dp[i][j] = 0;
	...
	if (count == 1)
		printf("only 1 char\n");
	else if (count == 2 && help[0] == help[1])	
		printf("only 2 same chars");
	
	//最终j - i 代表下标起始结束位置，以1开始
	for (i = 0; i < count + 1; i++) {
		//所有的 j > i 是没有意义的
		for (j = 0; j < i; j++) {
			//dp[i][j] = (dp[i + 1][j - 1] == 1 && \
				help[i - 1] == help[j - 1]) ? 1 : dp[i][j];
			dp[j][i] = (help[i - 1] == help[j - 1] && \
				(i - j < 2 || dp[j + 1][i - 1])) ? 1 : 0;	
		}
	}
	//验证
	for (i = 0; i < count + 1; i++) {
...
//Manachar计算Palindrome
void search_max_length_palindrome_str2(char* str)
{
	//创建范围
	int count = 0, flag = 0, i = 0, t = 0;
	int max_po = -1, max_p = -1;
	char* my_str = str;
	while (*my_str++ != '\0')
		count++;
	my_str = str;
	char* new_str = \
		(char*)malloc(sizeof(char) * (count * 2 + 1));
	char* length_str = \
		(char*)malloc(sizeof(char) * (count * 2 + 1));		
	...
	for (i = 0; i < count * 2 + 1; i++) {
		if (i <= max_p) {
			//i相对po的对称点
			int j = 2 * max_po - i;
			length_str[i] = length_str[j];
			if (length_str[j] >= max_p - i) 
				//从i + p 开始手动匹配
				for (t = length_str[i]; \
					((i + t) < (count * 2 + 1)) && (i - t >= 0); t++)
					if (new_str[i - t] == new_str[i + t])
						length_str[i]++;		
					else
						break;					
		} else {
				for (t = 1; ((i + t) < (count * 2 + 1)) && \
					(i - t >= 0); t++)
					if (new_str[i - t] == new_str[i + t])
						length_str[i]++;
					else
		...
		//更新最右值
		if (max_p < i + length_str[i]) {
			max_po = i;
			max_p = i + length_str[i];
		}	
	}	
...
//字符串转为整数
int charge_string_to_integer(char* str)
{
	int data = 0, flag = 0;
	while (*str != '\0') {
		if (*str == '-') 
...
//求装水的容器最大的大小
//依据数学的不等式原理设定的算法
int container_with_most_water(int* list, int left, int right)
{
	int buf_left = left, buf_right = right;
	int max_container = 0;
	int max_left = left, max_right = right;
	
	while (buf_left < buf_right) {
		int sum = (buf_right - buf_left)* \
			(list[buf_left] <= list[buf_right] ? list[buf_left] : list[buf_right]);
			
		if (sum > max_container) {
			max_container = sum;
			max_left = buf_left; 
			max_right = buf_right;		
		}	
		if (list[buf_left] <= list[buf_right])
			buf_left++;
		else
			buf_right--;	
	}
	printf("%d - %d - %d", max_left, max_right, max_container);
	return max_container;
}

//最长共有前缀
int longest_commer_prefix(char** list, int left, int right)
{
...
//三数和为x
void search_three_number_sum_equal_x(int* list, int left, int right, int x)
{
	int buf_left = left, buf_right = right;
	int index = left;
	//先排序其数组
	...
	//然后一个个数剔除
	for (index = left; list[index] < x; index++) {
		int target_sum = x - list[index];
		//跳过重复数据
		if ((index - 1 >= left) && list[index - 1] == list[index])
			continue;
		...
		while(buf_left < buf_right) {
			if ((buf_left - 1 >= left) &&  \
				list[buf_left - 1] == list[buf_left])
				buf_left++;
			if ((buf_right + 1 <= right) &&  \
				list[buf_right] == list[buf_right + 1])
				buf_right--;			
			
			if ((list[buf_left] + list[buf_right]) < target_sum)
				buf_left++;
			else if ((list[buf_left] + list[buf_right]) > target_sum)
				buf_right--;
			else {
				printf("%d - %d - %d \n", \
					list[index], list[buf_left], list[buf_right]);
				buf_left++;
				buf_right--;
			}
		}			
...
//求三数和与x相距最近
void search_three_number_sum_closest_x(int* list, int left, int right, int x)
{
	int buf_left = left, buf_right = right;
	int index = left;
	int closest_sum = ~(1 << 31);
	int index1 = left, index2 = left, index3 = left; 
	//先排序其数组
	quick_sort(list, left, right);
	//然后一个个数剔除
	for (index = left; index <= right; index++) {
		...
		while(buf_left < buf_right) {
			...
			buf_last = buf_sum;
			buf_sum = list[index] + list[buf_left] + list[buf_right];		
			if (abs(x - closest_sum) > abs(x - buf_sum)) {
				closest_sum = buf_sum;
				index1 = index;
				index2 = buf_left;
				index3 = buf_right;
	...
	printf("%d - %d - %d", list[index1], list[index2], list[index3]);
	
}

void search_four_number_sum_equal_x(int* list, int left, int right, int x)
{
	int buf_left = left, buf_right = right, target_sum, target_sum1;
	int index_level1 = left, index_level2 = left;
	//先排序其数组
	quick_sort(list, left, right);
	//然后一个个数剔除
	for (index_level1 = left; list[index_level1] < x; index_level1++) {
		target_sum = x - list[index_level1];
		//跳过重复数据
		if ((index_level1 - 1 >= left) && list[index_level1 - 1] == list[index_level1])
			continue;	
		for (index_level2 = index_level1; list[index_level2] < x; index_level2++) {
			target_sum1 = target_sum- list[index_level2];
			//跳过重复数据
			if ((index_level2 - 1 >= left) && list[index_level2 - 1] == list[index_level2])
				continue;			
			buf_left = index_level2 + 1;
			buf_right = right;
			while(buf_left < buf_right) {
				if ((buf_left - 1 >= left) &&  \
					list[buf_left - 1] == list[buf_left])
				...
				if ((list[buf_left] + list[buf_right]) < target_sum1)
					buf_left++;
				else if ((list[buf_left] + list[buf_right]) > target_sum1)
					buf_right--;
				else {
					printf("%d # %d # %d # %d \n", \
						list[index_level1], list[index_level2], list[buf_left], list[buf_right]);
					buf_left++;
					buf_right--;
				}
			}			

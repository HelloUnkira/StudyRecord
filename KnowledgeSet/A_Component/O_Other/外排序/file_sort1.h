#ifndef __FILE_SORT1_H_
#define __FILE_SORT1_H_
...
void multicannel_file_merge_sort_1(int n, ...)
{
	//读取需要归并的文件，数量为n，
	//将其指针描述符存入指针数组之中
...
//和上一函数功能一致，只是参数不同而已
void multicannel_file_merge_sort_2(FILE** fp_all, int left, int right)
{
...
int main(int argc, char* argv[])
{
	...
	//因为是使用变参函数，所以没有加入到程序中
	//另外替代的方式是，传入一个FILE*类型的数组，
	//已及左右端点
	multicannel_file_merge_sort(10, fp[0], fp[1], \
...
int main(int argc, char* argv[])
{
	...
	multicannel_merge_file_sort(2, fp1, fp2);
	//two_file_merge_sort(fp1, fp2);

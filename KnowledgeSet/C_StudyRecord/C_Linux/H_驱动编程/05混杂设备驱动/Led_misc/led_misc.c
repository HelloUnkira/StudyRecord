...
{
	printk(KERN_WARNING "led miscdevice is opened\n");
	return 0;
}

static int x6818_led_release(struct inode *inode, struct file *filp)
{
	printk(KERN_WARNING "led miscdevice is closed\n");
	return 0;
}
...
//1.定义一个字符操作集
static const struct file_operations x6818_led_misc_fops = {
	.owner = THIS_MODULE,
	.open = x6818_led_open,
	.read = x6818_led_read,
...
//2.定义一个混杂设备，初始化它
static struct miscdevice x6818_led_misc = {
	.minor = MISC_DYNAMIC_MINOR,
	.name = "led_misc",
	.fops = &x6818_led_misc_fops,
};
...
static int __init led_init(void)
{
	int ret;
	//3.注册一个混杂设备
	ret = misc_register(&x6818_led_misc);
	if (ret < 0) {
	...
	printk(KERN_WARNING "x6818 led miscdevice init \n");	
	return 0;
...
request_mem_region_C_fail:
	misc_deregister(&x6818_led_misc);
	return -EBUSY;
}
...
static void __exit led_exit(void)
{
	...
	release_mem_region(0xC001C000, 0x1000);	
	misc_deregister(&x6818_led_misc);
	printk(KERN_WARNING  "x6818 led miscdevice exit \n");		
}
...
MODULE_DESCRIPTION("LED miscdevice for X6818");
MODULE_LICENSE("GPL");
MODULE_VERSION("V1.0");
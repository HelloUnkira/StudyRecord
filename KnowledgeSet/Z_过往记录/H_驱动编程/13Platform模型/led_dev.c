...
static void __init led_virtual_address_init(void)
{
	//得到每个寄存器的虚拟地址
...
//platform driver的安装函数--->安装驱动
//struct platform_device *pdev --->与本driver匹配的device
static int __devinit x6818_led_probe(struct platform_device *pdev)
{
	int ret;
	//3、给字符设备申请一个设备号
	if (led_major == 0)
	...
	//8、从platform device中，获取resource，资源的类型是IORESOURCE_MEM，资源序号是0
	led_mem_0 = platform_get_resource(pdev, IORESOURCE_MEM, 0);
	if (led_mem_0 == NULL) {
		printk("no memory resource specified\n");
		ret = -ENOENT;
		goto platform_get_resource_0_fail;
	}
	
	//从platform device中，获取resource，资源的类型是IORESOURCE_MEM，资源序号是0
	led_mem_1 = platform_get_resource(pdev, IORESOURCE_MEM, 1);
	if (led_mem_1 == NULL) {
		printk("no memory resource specified\n");
		ret = -ENOENT;
		goto platform_get_resource_1_fail;
	}	
	
	
	//9、申请物理内存区，申请SFR的地址区。
	//SFR --- Special Function Register： GPIOEOUT
	leds_res_c = request_mem_region(led_mem_0->start, \
		led_mem_0->end - led_mem_0->start + 1, "GPIOC_MEM");
	if (leds_res_c == NULL) {
	...
	leds_res_e = request_mem_region(led_mem_1->start, \
		led_mem_1->end - led_mem_1->start + 1, "GPIOE_MEM");
	if (leds_res_e == NULL)	{ 
		printk(KERN_WARNING "request memory error\n");	
		ret = -EBUSY;
...
ioremap_C_fail:
	release_mem_region(led_mem_1->start, \
		led_mem_1->end - led_mem_1->start + 1);	
request_mem_region_E_fail:
	release_mem_region(led_mem_0->start, \
		led_mem_0->end - led_mem_0->start + 1);	
request_mem_region_C_fail:

platform_get_resource_1_fail:
platform_get_resource_0_fail:
...
static int __devexit x6818_led_remove(struct platform_device *dev)
{
...
//定义一个platform driver
static struct platform_driver x6818_led_driver = {
	.probe		= x6818_led_probe, //platform driver的初始化函数
	.remove		= __devexit_p(x6818_led_remove),//platform driver的卸载函数
	.driver		= {
		.owner	= THIS_MODULE,
		.name	= "x6818_led", //platform driver的名字与platform device的名字一致
	},
};

static int __init x6818_led_init(void)
{
	//platform driver的注册	
	return platform_driver_register(&x6818_led_driver);
}

static void __exit x6818_led_exit(void)
{
	//platform driver的注销
	platform_driver_unregister(&x6818_led_driver);		
}

//驱动程序出入口:insmod, rmmod
module_init(x6818_led_init);
module_exit(x6818_led_exit);
